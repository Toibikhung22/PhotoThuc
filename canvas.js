document.addEventListener('DOMContentLoaded', () => {
    ...
    // async function showCountdown() {
    //     const countdownText = document.getElementById("countdownText"); // Make sure you have this in your HTML
    //     countdownText.style.display = "block";
    
    //     // Get the selected countdown duration (default to 3 if none is selected)
    //     const timerOptions = document.getElementById("timerOptions");
    ...
    function handleImageUpload(event) {
        ...
        if (images.length >= 3) {
            const confirmReplace = confirm("You already have 3 pictures. Uploading a new image will replace all current pictures. Do you want to proceed?");
            
            if (!confirmReplace) {
                // Cancel upload
                event.target.value = ""; // reset input so same file can be uploaded again later
                return;
            }
            ...
            photoContainer.innerHTML = '';
            progressCounter.textContent = "0/3";
            startBtn.innerHTML = 'Capturing...'; // reset button text
            doneBtn.style.display = 'none';
        }
